...
async function main() {
  ...
  server.setRequestHandler('tools/list', async () => {
    ...
    if (brain) {
      tools.push({ name: 'dsg_propose_plan', description: 'Propose execution plan', inputSchema: {} as any });
    }
...
Phase: 3 (Testing) & 4 (Evaluation)`,
    ...
    return { isError: true, contents: [{ uri: request.params.uri, mimeType: 'text/plain', text: 'Not found' } as any] };
  });
