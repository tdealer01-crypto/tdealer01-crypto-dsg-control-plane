...
export class VercelService {
  ...
  async listDeployments(projectId: string, limit: number = 10): Promise<any[]> {
    try {
  ...
  async getBuildLogs(deploymentId: string, lines: number = 100): Promise<string> {
    try {
      const params = new URLSearchParams();
      if (this.teamId) params.append('teamId', this.teamId);
      ...
      return logs
        .map((log: any) => log.message || log.text || String(log))
        .slice(-lines)
        .join('\n');
    } catch (error) {
