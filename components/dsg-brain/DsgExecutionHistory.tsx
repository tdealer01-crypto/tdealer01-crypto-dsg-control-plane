...
interface DsgExecutionHistoryProps {
  ...
  onCompare?: (before: ExecutionHistoryEntry, after: ExecutionHistoryEntry) => void;
}

export default function DsgExecutionHistory({
  entries,
  onSelectEntry,
  onCompare,
}: DsgExecutionHistoryProps) {
  const [selectedForComparison, setSelectedForComparison] = useState<ExecutionHistoryEntry | null>(null);
  const [comparingSelectorFor, setComparingSelectorFor] = useState<ExecutionHistoryEntry | null>(null);

  const handleCompareSelect = (before: ExecutionHistoryEntry, after: ExecutionHistoryEntry) => {
    if (onCompare) {
      onCompare(before, after);
    }
    setComparingSelectorFor(null);
  };
        ...
        {entries.length === 0 ? (
          ...
          <div className="divide-y divide-slate-700">
            {entries.map((entry) => (
              <div
                key={entry.id}
                className="p-3 hover:bg-slate-800 transition border-b border-slate-700 last:border-b-0"
              >
                ...
                <div className="flex gap-1">
                  {comparingSelectorFor?.id === entry.id && (
                    <ComparisonSelector
                      currentEntry={entry}
                      availableEntries={entries}
                      onSelect={(selected) => handleCompareSelect(entry, selected)}
                      onCancel={() => setComparingSelectorFor(null)}
                    />
                  )}
                  {comparingSelectorFor?.id !== entry.id && (
                    <button
                      onClick={() => setComparingSelectorFor(entry)}
