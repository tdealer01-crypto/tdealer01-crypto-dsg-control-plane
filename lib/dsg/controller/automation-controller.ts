...
function highestRisk(
  a: DeterministicRiskLevel,
  b: DeterministicRiskLevel,
...
function riskFromClassification(
  classification: DsgAutomationControllerRequest["resource"]["classification"],
): DeterministicRiskLevel {
...
function riskFromAction(
  actionType: DsgAutomationControllerRequest["actionType"],
): DeterministicRiskLevel {
...
function evidenceStates(
  ...
  return Array.isArray(request.evidence)
    ? request.evidence.map((item) => item.state)
    : [];
}

function hasBlockingEvidence(request: DsgAutomationControllerRequest) {
  return evidenceStates(request).some(
    (state) => state === "BLOCKED" || state === "UNSUPPORTED",
  );
}
...
function hasOnlySafeEvidenceStates(request: DsgAutomationControllerRequest) {
  ...
  return states.every(
    (state) => state === "VERIFIED" || state === "REPO_STATED",
  );
}
...
function approvalRequired(
  ...
  if (
    request.resource.classification === "secret" ||
    request.resource.classification === "top_secret"
  )
    return true;
  return false;
}
...
function evaluatePolicy(
  ...
  riskLevel = highestRisk(
    riskLevel,
    riskFromClassification(request.resource.classification),
  );
...
function buildGateContext(
  ...
  const evidenceIsUsable =
    hasOnlySafeEvidenceStates(request) && !hasBlockingEvidence(request);
  const permissionGranted =
    asBoolean(request.context, "permission_granted") &&
    policy.failureReasons.length === 0;
...
function remediationFor(
  result: Pick<DsgAutomationControllerResult, "decision" | "policy">,
) {
...
export function evaluateAutomationController(
  ...
  return {
    ...
    evidenceBoundary: {
      ...
      externalSolverInvoked: gate.proof.evidenceBoundary.externalSolverInvoked,
