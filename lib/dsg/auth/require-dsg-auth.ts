...
async function resolveApiKey(rawKey: string): Promise<DsgCaller> {
  ...
  // TODO: Enable after migration 20260613000002 is applied
 ...
 *
 * TODO: Enable after migration 20260613000002 is applied (dsg_api_calls table)
 */
export async function logDsgApiCall(log: DsgCallLog): Promise<void> {
  // Disabled until dsg_api_calls table is created by migration
  try {
    // const admin = getSupabaseAdmin();
    // await admin.from('dsg_api_calls').insert({...});
    console.debug('[dsg-auth] logDsgApiCall: dsg_api_calls table not yet available');
  } catch (e) {
