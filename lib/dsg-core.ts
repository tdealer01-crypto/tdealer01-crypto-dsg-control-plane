...
function synthesizeMetricsFromExecutions(data: any) {
  ...
  const decisions = data.map((row) => String(row?.decision || row?.result?.decision || "").toUpperCase());
...
export async function getDSGCoreMetrics() {
  ...
  return {
    ok: false,
    error: metricsResult.error || executionsResult.error || "Failed to fetch DSG core metrics",
  };
}
