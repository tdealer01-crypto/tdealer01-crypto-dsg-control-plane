...
export function validateDeFiTransaction(tx: DeFiTransaction): DeFiValidationResult {
  ...
  if (tx.amountUSD > constraints.defi.maxSingleTxUSD)
    return { ok: false, reason: 'exceeds_single_tx_limit', field: 'amountUSD', bound: constraints.defi.maxSingleTxUSD };

  if (tx.dailySpentUSD + tx.amountUSD > constraints.defi.maxDailyUSD)
    return { ok: false, reason: 'exceeds_daily_limit', field: 'dailySpentUSD', bound: constraints.defi.maxDailyUSD };

  if (tx.slippageBps > constraints.defi.maxSlippageBps)
    return { ok: false, reason: 'exceeds_max_slippage', field: 'slippageBps', bound: constraints.defi.maxSlippageBps };

  if (!constraints.defi.allowedTokens.includes(tx.tokenIn))
    return { ok: false, reason: 'token_not_whitelisted', field: 'tokenIn', bound: constraints.defi.allowedTokens };

  if (!constraints.defi.allowedTokens.includes(tx.tokenOut))
    return { ok: false, reason: 'token_not_whitelisted', field: 'tokenOut', bound: constraints.defi.allowedTokens };

  if (!constraints.defi.allowedProtocols.includes(tx.protocol))
    return { ok: false, reason: 'protocol_not_whitelisted', field: 'protocol', bound: constraints.defi.allowedProtocols };
