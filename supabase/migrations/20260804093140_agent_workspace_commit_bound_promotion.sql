...
-- The previous function accepted a promotion ID but did not compare the
-- executor's commit to promotion.commit_sha.
...
-- Remove the older overload so service-role callers cannot bypass commit
-- binding by omitting p_commit_sha.
