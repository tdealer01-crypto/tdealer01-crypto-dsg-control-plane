...
begin
  ...
  insert into public.runtime_truth_states (
    ...
    runtime_commit_execution.truth_sequence,
    coalesce(p_created_at, now())
  ) returning id into truth_state_id;
  ...
  insert into public.runtime_ledger_entries (
    ...
    p_agent_id,
    p_request_id,
    runtime_commit_execution.execution_id,
    runtime_commit_execution.truth_state_id,
    upper(p_decision),
    p_reason,
    coalesce(p_metadata, '{}'::jsonb),
    p_canonical_hash,
    runtime_commit_execution.ledger_sequence,
    coalesce(p_created_at, now())
  ) returning id into ledger_id;

  v_checkpoint_hash := encode(
    extensions.digest(
      coalesce(p_canonical_hash, '') || ':' || runtime_commit_execution.ledger_id::text || ':' || runtime_commit_execution.truth_sequence::text,
      'sha256'
    ),
    'hex'
  );
  ...
  insert into public.runtime_checkpoints (
    ...
    p_agent_id,
    runtime_commit_execution.truth_state_id,
    runtime_commit_execution.ledger_id,
    v_checkpoint_hash,
    jsonb_build_object('source', 'runtime_commit_execution', 'execution_id', runtime_commit_execution.execution_id),
    coalesce(p_created_at, now())
