-- Apply repo migration supabase/migrations/20260702000000_add_monitoring_tables.sql
-- The monitoring feature (merged 2026-07-02) shipped without this being applied,
-- causing GET /api/monitoring/metrics to 500 ("Failed to load metrics" on dashboard).
...
DROP POLICY IF EXISTS "monitoring_executions_org_isolation" ON public.monitoring_executions;
...
DROP POLICY IF EXISTS "monitoring_events_read" ON public.monitoring_events;
...
DROP POLICY IF EXISTS "monitoring_tool_calls_read" ON public.monitoring_tool_calls;
...
DROP POLICY IF EXISTS "monitoring_token_usage_read" ON public.monitoring_token_usage;
