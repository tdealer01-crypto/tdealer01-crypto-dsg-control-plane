-- Add quota enforcement inside the atomic RPC transaction.
-- This prevents concurrent requests from exceeding quota limits.
...
begin
  ...
  if p_agent_monthly_limit > 0 then
    select coalesce(sum(uc.executions), 0) into v_agent_executions
  ...
  if p_org_plan_limit > 0 then
    select coalesce(sum(uc.executions), 0) into v_org_executions
    from usage_counters uc
  ...
  values (p_org_id, p_agent_id, p_decision, greatest(coalesce(p_latency_ms, 0), 0), coalesce(p_request_payload, '{}'::jsonb), coalesce(p_context_payload, '{}'::jsonb), p_policy_version, p_reason, coalesce(p_created_at, now()))
  ...
  insert into runtime_ledger_entries (org_id, agent_id, request_id, execution_id, truth_state_id, decision, truth_sequence, reason, metadata)
  values (p_org_id, p_agent_id, p_request_id, v_execution.id, v_truth.id, p_decision, v_truth_sequence, p_reason, coalesce(p_metadata, '{}'::jsonb))
  ...
  values (p_org_id, p_agent_id, v_execution.id, p_policy_version, p_decision, p_reason, coalesce(p_audit_evidence, '{}'::jsonb), coalesce(p_created_at, now()));

  insert into usage_events (org_id, agent_id, execution_id, event_type, quantity, unit, amount_usd, metadata, created_at)
  values (p_org_id, p_agent_id, v_execution.id, 'execution', 1, 'execution', coalesce(p_usage_amount_usd, 0), coalesce(p_metadata, '{}'::jsonb), coalesce(p_created_at, now()));

  insert into usage_counters (org_id, agent_id, billing_period, executions, updated_at)
  values (p_org_id, p_agent_id, to_char(coalesce(p_created_at, now()), 'YYYY-MM'), 1, coalesce(p_created_at, now()))
