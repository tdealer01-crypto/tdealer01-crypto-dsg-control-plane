...
### DSG Decision Frame — truth-first Z3 skill integration

This frame governs Z3 solver skills and every DSG decision. It is a required operating rule. It does not verify any existing claim by itself.

#### 1. `verify(data)`

Treat data as unverified until evidence confirms it.

Required evidence includes the current source file, repository revision, test output, build output, runtime error, or deployment artifact. Record the evidence location and execution time.

Never invent a result. Never infer an implementation from a plan, TODO, screenshot, or historical document.

#### 2. `samadhi(attention, target)`

State the user outcome before selecting a system design.

Define the decision target, risk, success condition, and next action. Design the Z3 constraints only after this user-facing target is clear.

#### 3. `kilesa(new_data, verified=false)`

Unverified new data is a risk, not a fact.

Assign `UNVERIFIED` until reliable evidence confirms it. For medium-or-higher risk, map `UNVERIFIED` to `REVIEW` or `BLOCK`, never `PASS`.

#### 4. `parami(history)`

Improve decisions with accumulated execution evidence.

Use repeated test runs, builds, deployments, incidents, and solver results. Preserve failures and counterexamples. Do not convert a single successful run into a general reliability claim.

#### 5. User-benefit gate

Before returning a decision, answer these questions:

* What concrete benefit does the user receive?
* Is the workflow easier to complete?
* Where can the user observe the result?
* What is the next safe action?

If the answer is unclear, return `REVIEW` and request the missing evidence or decision.

#### 6. Truth boundary

Correctness takes priority over approval.

State failures, missing evidence, and limitations directly. Do not claim deployment, compliance, performance, or solver proof without current evidence. Do not store a false claim as system data.

#### Required decision record

Every Z3-integrated skill decision must retain:

* User goal, target, risk level, and policy version.
* Input hash, solver result, constraint version, and counterexample when present.
* Evidence references, execution timestamp, and environment.
* Decision: `PASS`, `REVIEW`, `BLOCK`, or `UNVERIFIED`.
* User-visible outcome and next safe action.
...
✅ Risk-based contingency planning (APPROVE\_FAST → APPROVE\_WITH\_REVIEW → BLOCK\_AND\_ESCALATE)\
...
* **DSG Action Gate** - Gate AI actions through deterministic policy evaluation
  ...
  * Input: action, context, risk\_level, policy\_version
...
* **DSG Compliance Evidence** - Generate compliance evidence packs with audit trails
  ...
  * Input: execution\_id, include\_proofs, include\_audit\_trail
...
* **DSG Formal Proof** - Validate policy constraints using Z3 SMT solver
  ...
  * Input: policy\_constraint, execution\_context, solver
...
For complete plugin documentation, see [`.codex-plugin/README.md`](.codex-plugin/)
...
* ✅ **role\_safety** — If decision == ALLOW, actor ∈ WRITE\_ROLES
* ✅ **plan\_safety** — If decision == ALLOW, org ∈ EXECUTION\_PLANS
* ✅ **approval\_safety** — If ALLOW + approval\_required, has valid token
* ✅ **audit\_completeness** — Decision ∈ {ALLOW, BLOCK, REVIEW}
* ✅ **non\_triviality** — System is not unconditionally blocking
...
* Status mapping: orchestrationStatus (APPROVED / REQUIRES\_REVIEW / BLOCKED)
...
* Fix status: READY / IN\_PROGRESS / BLOCKED
...
**See Also:** [Skills Documentation](skills/) for detailed examples, decision flows, and test instructions.
...
* ✅ **Policy Evaluation** — CRUD operations, rule evaluation (amount\_threshold, rate\_limit, time\_window), approval workflow
...
* `billing_customers` — Org-scoped read access (verified via auth.uid + org\_id)
...
* **Documentation:** Comprehensive guides in [skills/README.md](skills/) with decision flows and examples
...
| ----------------------------------------------------------------------------------- | ------------------------ | -------------------------- |
| [**Claude Code Skills Guide**](skills/)                                             | AI Engineers & Operators | 10 min                     |
