...
import { Zap, Send, Loader, AlertCircle, StopCircle } from 'lucide-react';
...
export function AgiAgentChat() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 'init-1',
      role: 'system',
      content: '⚡ AGI Agent ready. Ask me anything about AI, code, or governance.',
      timestamp: new Date(),
    },
  ]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await fetch('/api/dashboard/agi/chat', {
        method: 'POST',
        headers: {
  ...
  const handleStop = async () => {
    try {
      await fetch('/api/dashboard/agi/stop', {
        method: 'POST',
  ...
  return (
    <div className="flex flex-col h-screen bg-gradient-to-b from-purple-50 to-white">
      <div className="flex items-center gap-3 px-6 py-4 border-b border-purple-200 bg-white">
        <Zap className="w-6 h-6 text-purple-600" />
        <div>
          <h1 className="text-lg font-bold text-slate-900">AGI Agent</h1>
          <p className="text-sm text-slate-500">
            AI capabilities · Code generation · Problem solving
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-4">
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="bg-purple-50 text-slate-900 rounded-lg px-4 py-3 flex items-center gap-2">
      ...
      <div className="border-t border-purple-200 bg-white px-4 py-4">
        <div className="flex gap-3">
          <textarea
            ...
            className="flex-1 px-4 py-2 border border-purple-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 resize-none text-sm"
            rows={1}
        ...
        <p className="text-xs text-slate-500 mt-2">
          ⚡ AGI agent uses Qwen3-Coder for code generation, analysis, and problem-solving
        </p>
      </div>
    </div>
  );
