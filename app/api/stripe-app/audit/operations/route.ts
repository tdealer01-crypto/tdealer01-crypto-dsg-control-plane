...
async function getOrgIdsForUser(userId: string): Promise<string[]> {
  ...
  try {
    ...
    console.warn('[stripe-app/audit/operations] org lookup failed:', error);
  }
...
async function getStripeAccountIds(userId: string, cookieAccountId: string | null): Promise<string[]> {
  ...
  try {
    ...
    console.warn('[stripe-app/audit/operations] account lookup failed:', error);
  }
...
export async function GET() {
  ...
  try {
    ...
    const { data, error } = await (supabase as unknown as {
            ...
            order: (column: string, opts: { ascending: boolean }) => {
              limit: (count: number) => Promise<{
                data: Array<{
                  ...
                  dsg_decision: 'ALLOW' | 'BLOCK' | 'REVIEW' | null;
      ...
      .select('id, stripe_object_id, operation_type, dsg_decision, dsg_reason, dsg_proof, created_at')
    ...
    console.error('[stripe-app/audit/operations] failed:', error);
    return NextResponse.json({ message: 'Failed to load audit records' }, { status: 500 });
  }
}
