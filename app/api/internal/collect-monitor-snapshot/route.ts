...
async function recordAlertsForOrg(args: {
  orgId: string;
  snapshotId: number;
  alerts: MonitorAlert[];
}) {
  const { orgId, snapshotId, alerts } = args;
    ...
    if (existing?.id) {
      const { error: updateError } = await admin
        ...
        .update({
          ...
          context: { source_snapshot_id: snapshotId },
        })
        .eq("id", existing.id);
...
async function collectOrgSnapshot(args: {
  ...
  if (alerts.length > 0) {
    await recordAlertsForOrg({ orgId, snapshotId: snapshot.id, alerts });
  }
...
export async function GET(req: Request) {
  try {
    ...
    const snapshots = settled
      .filter((entry): entry is PromiseFulfilledResult<{ orgId: string; snapshotId: number; readiness: string; alerts: number }> => entry.status === "fulfilled")
      .map((entry) => entry.value);

    const failures = settled
      .filter((entry): entry is PromiseRejectedResult => entry.status === "rejected")
      .map((entry) =>
        entry.reason instanceof Error ? entry.reason.message : "Unknown org snapshot failure"
      );
