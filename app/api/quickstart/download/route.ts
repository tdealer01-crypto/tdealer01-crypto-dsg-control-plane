...
export async function GET(request: NextRequest) {
  ...
  // Read-only — do NOT rotate the key. The key was shown once during setup.
  // Rotating here would silently invalidate any key the user already copied.
  const starterAgent = await ensureStarterAgent(access.orgId, { rotateKey: false });
  const baseUrl = request.nextUrl.origin;

  // If the agent pre-exists, we can't retrieve the plain key (it's hashed).
  // Use a placeholder — the user should paste their key from the setup screen
  // or create a new one at /dashboard/api-keys.
  const apiKeyForFile = starterAgent.api_key || 'YOUR_DSG_API_KEY';

  const isPython = lang === 'python';
  const filename = isPython ? 'dsg_gate.py' : 'dsg_gate.js';
  const content = isPython
    ? pythonFile(baseUrl, apiKeyForFile, starterAgent.agent_id)
    : javascriptFile(baseUrl, apiKeyForFile, starterAgent.agent_id);
