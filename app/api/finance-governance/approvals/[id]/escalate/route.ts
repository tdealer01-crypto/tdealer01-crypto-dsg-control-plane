...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    void fireFinanceWebhook(orgId, 'finance.approval.escalated', {
      approval_id: id,
      next_status: typeof result === 'object' && result !== null && 'nextStatus' in result ? String((result as Record<string, unknown>).nextStatus) : 'escalated',
    });
