...
export async function GET(request: Request) {
  try {
    ...
    if (!orgId || !/^[a-f0-9\-]{36}$/.test(orgId)) {
      return NextResponse.json({ ok: false, error: 'org_id parameter required and must be valid UUID' }, { status: 400 });
    }
    ...
    const assertionConsumerServiceUrl = `${baseUrl}/api/auth/saml/callback?org_id=${encodeURIComponent(orgId)}`;
    const singleLogoutServiceUrl = `${baseUrl}/api/auth/saml/logout?org_id=${encodeURIComponent(orgId)}`;
