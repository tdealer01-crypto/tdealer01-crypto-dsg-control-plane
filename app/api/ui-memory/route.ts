...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ ok: true, payload: result.data?.payload ?? {}, updatedAt: result.data?.updated_at ?? null });
}
...
export async function DELETE(request: Request) {
  ...
  if (result.error) {
    if (isMissingMemoryTable(result.error)) return NextResponse.json({ ok: true, missingMigration: true });
    return NextResponse.json({ error: result.error.message }, { status: 500 });
  }
