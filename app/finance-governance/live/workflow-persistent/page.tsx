...
export default function FinanceGovernanceLiveWorkflowPersistentPage() {
  ...
  const counts = useMemo(
    () => deriveCounts(approvals, submittedCount),
    [approvals, submittedCount],
  );

  const refreshData = useCallback(
    async (showRefreshing = false, reset = false) => {
      try {
        if (showRefreshing) {
          setRefreshing(true);
        } else {
        ...
        const [workspaceResponse, approvalsResponse] = await Promise.all([
          financeGovernanceFetch("/api/finance-governance/workspace/summary", {
            cache: "no-store",
          }),
          financeGovernanceFetch("/api/finance-governance/approvals", {
            cache: "no-store",
          }),
        ]);

        const workspaceJson =
          (await workspaceResponse.json()) as WorkspaceSummaryResponse;
        const approvalsJson =
          (await approvalsResponse.json()) as ApprovalsResponse;
        ...
        setSubmittedCount(workspaceJson.workspace.counts.readyExports);
        if (reset) {
          setBanner({
            kind: "success",
            text: "State refreshed from the org-scoped API read model.",
          });
        }
      } catch (err) {
        setError(
          err instanceof Error
            ? err.message
            : "Failed to refresh workflow data",
        );
      } finally {
        setLoading(false);
        setRefreshing(false);
  ...
  async function runSubmit() {
    try {
      ...
      const response = await financeGovernanceFetch(
        "/api/finance-governance/submit",
        {
          method: "POST",
          headers: {
      ...
      setBanner({
        kind: "error",
        text:
          err instanceof Error ? err.message : "Failed to submit workflow item",
      });
    } finally {
      setBusyKey(null);
    }
  ...
  async function runApprovalAction(
    ...
    try {
      ...
      const response = await financeGovernanceFetch(
        `/api/finance-governance/approvals/${approvalId}/${action}`,
        {
          method: "POST",
        },
      );
    ...
    <main className="mx-auto min-h-screen max-w-7xl px-6 py-16 text-white">
      <div className="max-w-3xl">
        ...
        <h1 className="mt-4 text-4xl font-bold md:text-5xl">
          Workflow state from the control-layer API
        </h1>
        <p className="mt-6 text-lg leading-8 text-slate-300">
          This page reads action results from the finance-governance API and
          refreshes after every submit, approve, reject, or escalate operation.
          Browser storage is not used as source-of-truth.
        </p>
      </div>
      ...
      <div className="mt-8 flex flex-wrap gap-4">
        ...
        <button
          ...
          disabled={busyKey !== null || refreshing}
          className="rounded-2xl border border-emerald-400/30 bg-emerald-500/10 px-6 py-3 font-semibold text-emerald-100 disabled:opacity-70"
        >
          Reload org read model
        </button>
      </div>
      ...
      {loading ? (
        <div className="mt-10 rounded-[1.75rem] border border-white/10 bg-white/5 p-7 text-slate-200">
          Loading workflow data...
        </div>
      ) : null}
      {error ? (
        <div className="mt-10 rounded-[1.75rem] border border-red-500/30 bg-red-500/10 p-7 text-red-200">
          {error}
        </div>
      ) : null}
                ...
                {approvals.length === 0 ? (
                  ...
                  approvals.map((item) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-4 text-emerald-100">
                        {item.status}
                      </td>
