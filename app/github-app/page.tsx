...
const FEATURES = [
  { icon: '✅', title: 'PR Check Runs', body: 'DSG Gate appears as a required status check on every pull request — ALLOW or BLOCK based on policy.' },
  { icon: '📜', title: 'Audit Trail', body: 'Every gate decision is written to the DSG audit ledger with actor, verdict, timestamp, and PR context.' },
...
export default function GitHubAppPage() {
  ...
  const manifest = JSON.stringify({
    ...
    description: 'AI governance gate for pull requests — policy check, audit trail, deterministic ALLOW/BLOCK on every PR.',
    public: true,
    default_permissions: {
      checks: 'write',
      pull_requests: 'write',
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-100">
      <div className="mx-auto max-w-4xl px-6 py-16">
          ...
          <ol className="mt-5 space-y-3">
            {[
              ...
              <li key={i} className="flex items-start gap-3">
                <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-emerald-500 text-sm font-bold text-black">
                  {i + 1}
                </span>
        ...
        {/* Install button — GitHub manifest flow */}
        <div className="mt-10 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-8 text-center">
          ...
          <p className="mt-2 text-sm text-slate-400">
            Click below — GitHub will create the app in your account and redirect back here.
          </p>

          <form
            action="https://github.com/apps/manifests"
            method="post"
          ...
          <p className="mt-4 text-xs text-slate-500">
            Requires: checks (write) · pull_requests (write) · contents (read)
          </p>
        </div>
        ...
        <div className="mt-8 rounded-2xl border border-slate-800 bg-slate-900 p-6">
          ...
          <p className="mt-2 text-sm text-slate-400">Shown on the next page after GitHub creates your app.</p>
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-4">
          <Link href="/pricing" className="rounded-xl border border-slate-700 px-5 py-3 text-sm font-semibold text-slate-300 hover:border-emerald-400">
            View pricing →
          </Link>
          <Link href="/marketplace/production-evidence" className="rounded-xl border border-slate-700 px-5 py-3 text-sm font-semibold text-slate-300 hover:border-emerald-400">
            View evidence →
          </Link>
        </div>
      </div>
