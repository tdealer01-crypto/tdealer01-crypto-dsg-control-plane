...
function makeUserTable(profile: unknown) {
  return {
...
function makeSupabaseClient(user: unknown, profile: unknown) {
  return {
    ...
    from: vi.fn(() => makeUserTable(profile)),
  };
}
...
describe('POST /api/billing/checkout', () => {
  it('returns 429 when rate limit exceeded', async () => {
    ...
    mockCreateClient.mockResolvedValue(makeSupabaseClient({ id: 'user-1' }, { email: 'test@example.com' }) as any);
  ...
  it('allows checkout when user profile exists', async () => {
  ...
  it('returns 403 when profile row is missing even with authenticated user', async () => {
    mockCreateClient.mockResolvedValue(
      makeSupabaseClient(
        { id: 'user-1' },
        null
