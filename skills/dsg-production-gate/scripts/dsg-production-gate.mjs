...
if (!apiKey) fail('DSG_API_KEY is required. Create a key with gates:evaluate and/or proofs:prove scope.');
...
const timeout = setTimeout(() => controller.abort(), Number(args.timeout || 20000));
let response;
let body;
try {
  response = await fetch(`${baseUrl}${path}`, {
    ...
    headers: {
      ...
      'user-agent': 'dsg-production-agent-skill/1.0',
    },
...
if (!response.ok) {
  ...
  const action = response.status === 401 || response.status === 403
    ...
    : response.status === 402
      ? 'Quota/entitlement blocked this call; upgrade or correct entitlement.'
      : response.status === 429
