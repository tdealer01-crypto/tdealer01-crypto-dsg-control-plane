...
compatibility: >
  Requires Node.js 18+ and a DSG API key. Defaults to the DSG ONE production
  control-plane URL and never falls back to mock data.
description: >-
...
3. **Evidence** — when supplied by the API, the result includes `proofId`, `proofHash`, and `productionReadyClaim`.
4. **Fix** — authentication, entitlement, quota, rate-limit, malformed-response, and network failures return a concrete next action.
5. **Truth boundary** — no successful production claim is allowed unless the remote API actually returns it.
...
The repository implementation for these routes requires authenticated DSG callers, applies an org-scoped rate limit, checks gate entitlement, and records usage. Do not replace these calls with a local mock when the task asks for a production decision.

## Authentication

Create an API key from DSG ONE with the narrowest required scope:
...
Never print, persist, commit, or place the raw key in command history examples, evidence bundles, logs, issue bodies, or agent responses.

Optional override for staging/local development only:

```bash
export DSG_CONTROL_PLANE_URL='https://your-approved-host.example'
```
...
For larger context, pass a JSON file instead of shell JSON:
...
Treat the remote result as follows:

| Result | Agent action |
|---|---|
| `PASS` | May continue only when `ok=true`; preserve proof evidence. |
...
The CLI exit codes are intentionally machine-friendly:
...
- `402`: quota or entitlement blocked the request; fix entitlement or upgrade before retrying.
...
This skill is wired to the repository's production API contract. A successful live production invocation still depends on deployment reachability, a valid API key, active backing services, and current entitlements. If those are not directly verified in the current run, state that limitation rather than claiming the production call succeeded.
