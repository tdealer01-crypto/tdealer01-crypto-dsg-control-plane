...
metadata:
  hermes:
    tags:
      ...
      - governance
description: >-
  Remediation skill for shipping the DSG Governance Control Plane to the Claude
  Code plugin marketplace. Fixes the two remaining blockers after the
  marketplace.json owner merge: (1) the premature Claude
---

# DSG Marketplace Fix

Closes the remaining marketplace-launch blockers after `owner` was merged into `.claude-plugin/marketplace.json` (commit `b08d3fc`).
...
Expected: HTTP 200 with authorization-server metadata (issuer, authorization\_endpoint, token\_endpoint, revocation\_endpoint). If 404 → clear Vercel build cache and redeploy.
