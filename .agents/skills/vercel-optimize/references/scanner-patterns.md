...
**Description.** Nested .include({ x: { include: { y: { include: { z: ... } } } } }) makes Prisma issue a single huge join that scales O(&#x4E;_&#x4D;_&#x4B;). Function duration explodes, memory spikes, often causes timeouts.
...
**Description.** Four shapes of image-cost waste: raw  tags bypass the framework Image component; `images.unoptimized: true` disables Vercel image optimization globally;  without `sizes` forces serving the largest source variant;  without `unoptimized` routes vector data through the raster pipeline.
