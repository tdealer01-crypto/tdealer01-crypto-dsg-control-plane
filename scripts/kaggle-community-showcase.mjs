...
const PUBLISHING_BOUNDARY = {
  ...
  forbidden: [
    'DSG control-plane source zip',
    'full repository source package',
  ...
  note: 'Publish only the Kaggle Community task pack and generated showcase artifacts. Do not upload dsg-control-plane-main.zip or any full DSG core source package to Kaggle.',
};
...
async function main() {
  ...
  const checks = {
    ...
    noCoreSourcePublishingBoundaryPresent: PUBLISHING_BOUNDARY.forbidden.includes('DSG control-plane source zip'),
  ...
  const manifest = {
    ...
    summary,
    checks,
    publishingBoundary: PUBLISHING_BOUNDARY,
    sourceFiles: [
    ...
    kaggleSetup: {
      ...
      nextSteps: [
        ...
        'Publish only the task pack/report artifacts; do not upload DSG core source packages or production runtime code.',
      ],
    },
  ...
  await fs.writeFile(path.join(OUT_DIR, 'NO_CORE_SOURCE_BOUNDARY.md'), renderNoCoreSourceBoundary());
...
function renderReport(manifest) {
  ...
  const fileRows = manifest.sourceFiles
    ...
    .join('\n');
  const allowedRows = manifest.publishingBoundary.allowed.map((item) => `- ${item}`).join('\n');
  const forbiddenRows = manifest.publishingBoundary.forbidden.map((item) => `- ${item}`).join('\n');

  return `# DSG Kaggle Community Benchmark Showcase\n\nGenerated at: ${manifest.generatedAt}\n\n## Verdict\n\n**${manifest.summary.pass ? 'PASS' : 'FAIL'}**\n\n## Summary\n\n- Suite: ${manifest.summary.suiteId}\n- Tasks: ${manifest.summary.taskCount}\n- Cases: ${manifest.summary.caseCount}\n\n## No-core-source publishing boundary\n\n${manifest.publishingBoundary.note}\n\nAllowed to publish:\n\n${allowedRows}\n\nForbidden to publish:\n\n${forbiddenRows}\n\n## Task pack\n\n| Task ID | Name | Cases | Max score | Copy-paste ready |\n|---|---|---:|---:|---:|\n${taskRows}\n\n## Validation checks\n\n| Check | Result |\n|---|---:|\n${checkRows}\n\n## Source file hashes\n\n| File | Size bytes | SHA-256 |\n|---|---:|---|\n${fileRows}\n\n## Kaggle setup boundary\n\n- Task: ${manifest.kaggleSetup.taskDefinition}\n- Benchmark: ${manifest.kaggleSetup.benchmarkDefinition}\n\n## Next steps\n\n${manifest.kaggleSetup.nextSteps.map((step) => `- ${step}`).join('\n')}\n\n## Evidence boundary\n\n${manifest.evidenceBoundary}\n`;
}

function renderFileInformation() {
  return `# Kaggle file information copy\n\nUse these descriptions in Kaggle's Add file information panel.\n\n## README.md\nPrimary dataset documentation. Explains dataset purpose, benchmark task scope, local test evidence, provenance, DOI references, no-core-source boundary, and evidence boundary. Read this file first.\n\n## benchmarks/kaggle-community/benchmark_cases.json\nDeterministic Community Benchmark preparation cases for DSG financial governance tasks. Includes task IDs, prompts, expected gate status, risk level, required evidence terms, forbidden false-claim terms, and evidence boundary. Does not include DSG runtime source code.\n\n## benchmarks/kaggle-community/dsg_finance_governance_tasks.py\nKaggle task-ready Python scoring functions for Payment Decision Control, Policy Compliance Detection, and Audit Evidence Generation. Copy individual functions into Kaggle Benchmark task notebooks. This is scoring/task code only, not DSG control-plane source code.\n\n## artifacts/kaggle-community-showcase/kaggle-community-showcase-manifest.json\nGenerated manifest validating the Kaggle Community Benchmark task pack structure, source file hashes, task counts, case counts, no-core-source publishing boundary, and claim boundary.\n\n## artifacts/kaggle-community-showcase/kaggle-community-showcase-report.md\nHuman-readable showcase report summarizing tasks, validation checks, source hashes, Kaggle setup steps, no-core-source boundary, and evidence boundary.\n\n## artifacts/kaggle-community-showcase/README-kaggle-copy.md\nREADME text prepared for Kaggle dataset/notebook publication. It separates author-generated evidence from public Kaggle leaderboard results and states that DSG core source packages must not be uploaded.\n\n## artifacts/kaggle-community-showcase/NO_CORE_SOURCE_BOUNDARY.md\nExplicit publishing boundary stating that dsg-control-plane-main.zip, full repository source packages, runtime core source code, secrets, customer data, and raw production audit exports must not be uploaded to Kaggle.\n`;
}
